...
@HiltViewModel
internal class CurrenciesSettingsModel @Inject constructor(
    ...
    override fun swapPositions(item: CurrencyItem, anotherItem: CurrencyItem) {
        val map = buildMap {
            this[item] = anotherItem.copy(position = item.position)
            this[anotherItem] = item.copy(position = anotherItem.position)
        }
        displayedVisibleItems.replaceAll(map)
    }
