...
internal fun addRemoveBrackets(expression: String): String {
...
internal fun Calculations<*>.isBinaryOperationSymbol(operation: String): Boolean {
    return findTypedOperation<BinaryOperation<*>>(operation) != null
}

internal fun Calculations<*>.isBinaryOperationSymbol(c: Char): Boolean {
...
private inline fun <reified T : Operation<*>> Calculations<*>.findTypedOperation(textRepresentation: String): T? {
    return field.operations.filterIsInstance<T>().find { it.textRepresentations.contains(textRepresentation) }
}

internal fun Calculations<*>.hasSignsInExpression(expr: String): Boolean {
    for (c in expr.toCharArray()) {
...
internal inline fun <reified T : Operation<*>> Calculations<*>.startsWithOperation(text: String): Boolean {
    return field.operations.filterIsInstance<T>().any { operation ->
        operation.textRepresentations.any { text.startsWith(it) }
    }
}

fun localizeExpression(
    expression: String,
